...
namespace Collision
{
	...
	static BitmaskRegistry& bitmasks() {
		static BitmaskRegistry instance; // Correct initialization
		return instance;
	}
	...
	struct OrientedBoundingBox // Used in the BoundingBoxTest
	{
		...
		OrientedBoundingBox(const sf::Sprite& sprite) // Calculate the four points of the OBB from a transformed (scaled, rotated...) sprite
		{
			...
			points[1] = transform.transformPoint(local.width * 1.0f, 0.f);
			points[2] = transform.transformPoint(local.width * 1.0f, local.height * 1.0f);
			points[3] = transform.transformPoint(0.f, local.height * 1.0f);
		}
